...
class SectionedMultiSelect extends PureComponent {
  ...
  _getSelectLabel = () => {
    ...
    if (!selectedItems || selectedItems.length === 0) {
      ...
      if(selectedItems.length === this.props.merchants.length) {
        return "All Outlets";
      }
      ...
      <View>
        ...
        {this._showSubCategoryDropDown(item[uniqueKey]) &&
          <View>
            {item[subKey] &&
              <FlatList
                keyExtractor={i => String(i[uniqueKey])}
                data={item[subKey]}
          ...
          <View style={[{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }, this.styles.backdrop]}>
            ...
            >
              ...
              <View style={styles.switch}>
                <TouchableOpacity onPress={() => { this._removeAllItems() }} style={{ margin: 5 }}>
                  <Text>Clear All</Text>
                </TouchableOpacity>
                <View style={{ flex: 1 }} />
                <View style={{ margin: 5 }}>
                  <Text>Select Multiple Outlets</Text>
                </View>
                <Switch
                  onValueChange={() => this.props.toggleSwitch(this.props.single)}
